...
impl<EHF: EnvoyHttpFilter> HttpFilter<EHF> for Filter {
    fn on_request_body(
        ...
        println!("now: {:?}", data.iter().as_slice());
        println!("plz: {:?}", data.as_slice());

        let mut body_reader = BodyReader::new(data);
        println!("Body reader data: {:?}", body_reader);

        let payload = body_reader.get_str_payload();
...
impl<'a> BodyReader<'a> {
    ...
    fn get_str_payload(&self) -> String {
        let mut payload = String::new();
        for buf in self.data.iter() {
            let temp = buf.as_slice().to_owned();

            payload += &String::from_utf8(temp).unwrap();
        }
